...
TL;DR: Non-reciprocal interactions in high-dimensional systems induce chaotic attractors with non-monotonic Lyapunov exponent dependence on interaction asymmetry.


<details>
  <summary>Details</summary>
Motivation: To understand how varying degrees of interaction symmetry/non-reciprocity affect dynamics in high-dimensional systems, bridging between completely symmetric (gradient descent) and completely asymmetric (chaotic) regimes.

Method: Introduce parameter α tuning degree of non-reciprocity in interactions; analyze dynamics as function of α; examine maximal Lyapunov exponent behavior.

Result: Any non-zero non-reciprocity (α ≠ 0) leads to chaotic attractor; maximal Lyapunov exponent shows non-monotonic dependence on α; conservative forcing from gradient field can enhance chaos.

Conclusion: Partial symmetry breaking induces chaos in high-dimensional systems, with non-monotonic relationship between non-reciprocity and chaos intensity, revealing that gradient forces can amplify chaotic behavior.
...
TL;DR: Non-Hermitian spinful AAH model with Rashba SOC and textured magnetic field produces butterfly spectral map with asymmetric spin alignment, arising from combined effects of non-Hermiticity, SOC, and magnetic texture.


<details>
  <summary>Details</summary>
Motivation: To study the interplay between non-Hermiticity, spin-orbit coupling, and textured magnetic fields in the spinful Aubry-André-Harper model, and understand their combined effects on spectral properties and localization.

Method: Analysis of the Non-Hermitian spinful Aubry-André-Harper model incorporating Rashba-type spin-orbit coupling and a spatially varying textured magnetic field.

Result: Produces a butterfly spectral map with non-trivial state localization extent, exhibiting asymmetric spin alignment with respect to butterfly wings, arising from combined effects of non-Hermiticity, spin-orbit interaction, and textured magnetic field.

Conclusion: The butterfly spectral map with asymmetric spin alignment emerges from the synergistic combination of non-Hermiticity, spin-orbit coupling, and textured magnetic field in the spinful AAH model.
...
TL;DR: Spatial averaging in MBL diagnostics masks microscopic features; site-resolved spin autocorrelators reveal complex dynamics, resonant structures, and rare local instabilities within the MBL phase, explained by few-site models.


<details>
  <summary>Details</summary>
Motivation: To explore limitations of using imbalance dynamics as a diagnostic tool for many-body localization (MBL) and demonstrate that spatial averaging can obscure important microscopic features in strongly disordered systems.

Method: State-of-the-art numerical techniques (Krylov time evolution and full diagonalization) applied to the random-field XXZ chain, analyzing site-resolved spin autocorrelators and their time evolution/infinite-time limits, supported by analytical few-site toy models.

Result: Site-resolved spin autocorrelators reveal rich complex dynamical behavior obscured by imbalance observable, showing resonant structures and rare local instabilities within MBL phase; few-site models capture multiple-peak structure in local magnetization histograms as hallmark of local resonances.

Conclusion: Few-body local effects provide detailed understanding of ergodicity-breaking dynamics, explain finite-size effects of long-time imbalance and sensitivity to initial conditions; highlights necessity of refined site-resolved approach for understanding MBL complexities and rare-region effects.
...
TL;DR: The paper studies false vacuum decay dynamics in 2+1D quantum Ising model using tree tensor networks to simulate bubble nucleation and evolution, with applications to quantum simulation on Rydberg arrays.


<details>
  <summary>Details</summary>
Motivation: Understanding false vacuum decay dynamics in higher spatial dimensions is a major challenge in particle physics and cosmology, requiring new numerical approaches to study microscopic bubble nucleation processes.

Method: Uses tree tensor networks to simulate microscopic dynamics of spin-down domains in spin-up backgrounds in 2+1D quantum Ising model with longitudinal field, treating domains as true vacuum bubbles in false-vacuum background.

Result: Identifies how bubble fate (expansion or collapse) depends on geometrical features and microscopic Hamiltonian parameters, and provides realistic quantum simulation scheme using Rydberg atom arrays.

Conclusion: The study advances understanding of false vacuum decay dynamics in higher dimensions through quantum simulation approaches, bridging fundamental physics with experimental quantum platforms.
...
TL;DR: Quantum error correction enables rare signal detection by discriminating signal from noise through higher-order correlations during syndrome extraction, improving sensitivity over conventional methods.


<details>
  <summary>Details</summary>
Motivation: To develop a protocol for detecting rare signals in noisy environments where conventional sensing strategies have limited sensitivity due to noise interference.

Method: Uses quantum error correction (QEC) to discriminate signals from noise through distinct higher-order correlations during syndrome extraction. The protocol sacrifices part of the signal but corrects physical noise and extends logical coherence time for signal acquisition.

Result: Demonstrates improved sensitivity for detecting rare signals occurring at random times in the presence of local Markovian noise compared to conventional sensing strategies.

Conclusion: QEC-based sensing provides a novel approach for rare signal detection by leveraging the non-linear processing inherent in syndrome extraction to distinguish signals from noise through higher-order correlations.
...
TL;DR: Quantum sensing using entangled many-body states, particularly quantum critical wavefunctions, can surpass standard quantum limits. The paper develops interferometric sensing protocols, introduces a symmetry-based algorithm for optimal measurements, and demonstrates regimes where critical systems outperform GHZ states, even showing that non-unitary deformation can enhance precision.


<details>
  <summary>Details</summary>
Motivation: To develop quantum sensing protocols that go beyond the standard quantum limit by leveraging entangled many-body states, specifically quantum critical wavefunctions, and to systematically identify optimal measurement strategies using symmetries as a metrological resource.

Method: Developed interferometric sensing protocols based on quantum critical wavefunctions; introduced a symmetry-based algorithm to identify optimal measurement strategies; applied the algorithm to magnetic systems with internal symmetries and Rydberg-atom arrays with spatial symmetries; studied robustness under non-unitary deformations, symmetry-preserving/breaking decoherence, and qubit loss.

Result: Identified regimes where critical systems outperform GHZ states and spin-squeezed states; showed that non-unitary deformation can enhance sensing precision; demonstrated the algorithm's effectiveness for both internal and spatial symmetry systems; found that criticality remains robust for quantum sensing under various perturbations.

Conclusion: Interferometric sensing using quantum critical wavefunctions is a promising approach for high-precision quantum sensing, especially when combined with recent advances in log-depth preparation of critical states. The symmetry-based algorithm provides a systematic way to optimize measurements, and critical systems can outperform traditional entangled states in certain regimes.
...
TL;DR: VQA solves 1D Bratu equation using quantum neural network parameterization, accurately capturing both solution branches with results matching classical continuation methods.


<details>
  <summary>Details</summary>
Motivation: To develop a quantum computing approach for solving nonlinear differential equations, specifically the Bratu equation, which has two solution branches and serves as a benchmark problem for nonlinear BVPs.

Method: Formulate Bratu equation as variational problem, encode solution in parameterized quantum neural network (QNN), incorporate classical approximations and boundary-enforcing terms in trial solution, optimize quantum circuit parameters to minimize differential operator residual.
...
Conclusion: VQA with QNN encoding provides effective quantum approach for solving nonlinear BVPs, demonstrating potential for quantum advantage in differential equation solving.
...
TL;DR: QUBO penalty weights for Job Shop Scheduling act as thermodynamic control knobs, with sharp transitions in feasibility and solver success that correlate with reorganization of dissipation and thermodynamic efficiency in quantum annealing.


<details>
  <summary>Details</summary>
Motivation: Different QUBO encodings for constrained optimization problems reshape the energy landscape experienced by quantum annealers, but the relationship between penalty choices, computational hardness, and thermodynamic behavior remains unexplored.

Method: Study Job Shop Scheduling using a two-parameter family of QUBO encodings controlled by penalty weights p_sum (one-hot constraints) and p_pair (precedence constraints). Sweep penalty parameters, analyze feasibility and solver success across classical heuristics and D-Wave Advantage processor. Treat annealer as open thermodynamic system, perform cyclic reverse-annealing experiments initialized from thermal samples, measure stochastic processor energy change, and infer entropy production, work, and exchanged heat via thermodynamic uncertainty relations. Corroborate with adiabatic master equation simulations.

Result: Observe sharp transitions in feasibility and solver success across penalty parameter space. Weak penalties generate low-energy infeasible manifolds, while overly strong penalties suppress effective problem energy scale and increase irreversibility. Encoding transitions that govern computational hardness also reorganize dissipation, reducing thermodynamic efficiency at penalty extremes.

Conclusion: QUBO penalty weights serve as thermodynamic control knobs, motivating thermodynamics-aware encoding strategies for noisy intermediate-scale quantum annealers to optimize both computational success and thermodynamic efficiency.
...
TL;DR: The paper presents a systematic four-step quantization procedure for superconducting circuits using the Schur complement of the nodal admittance matrix, which naturally leads to eigenvalue-dependent boundary conditions for dressed mode spectra, ensuring UV convergence without imposed cutoffs.


<details>
  <summary>Details</summary>
Motivation: To develop a rigorous quantization framework for superconducting circuits that avoids ad hoc cutoffs and provides explicit validity conditions for standard circuit QED parameters, addressing the need for systematic treatment of multiport electromagnetic environments in circuit quantum electrodynamics.

Method: A four-step quantization procedure: (1) compute/measure driving-point admittance Y_in(s) via Schur complement of nodal admittance matrix, (2) solve boundary condition sY_in(s) + 1/L_J = 0 for dressed frequencies, (3) synthesize equivalent passive network, (4) quantize with full cosine nonlinearity retained. Uses passive lumped-element circuit theory to prove junction participation decay properties.

Result: Proves that junction participation decays as O(ω_n^{-1}) at high frequencies when the junction port has finite shunt capacitance, ensuring ultraviolet convergence of perturbative sums without imposed cutoffs. Derives standard circuit QED parameters (coupling strength g, anharmonicity α, dispersive shift χ) as controlled limits with explicit validity conditions.

Conclusion: The Schur complement approach provides a systematic, mathematically rigorous quantization framework for superconducting circuits that naturally handles multiport electromagnetic environments, ensures UV convergence without artificial cutoffs, and yields standard circuit QED parameters with explicit validity conditions, offering a complete quantization procedure from classical circuit analysis to quantum description.
...
TL;DR: TN-Sim is a tensor network simulator backend for quantum circuit simulation that integrates with TAMM framework to enable scalable HPC simulations, demonstrating performance on Perlmutter supercomputer with potential for cross-platform portability.


<details>
  <summary>Details</summary>
Motivation: Large-scale tensor network simulations are essential for establishing complexity-theoretic bounds on classical quantum simulation, enabling circuit cutting techniques, and optimizing circuit compilation to facilitate efficient quantum computation on limited quantum resources. Modern exascale HPC platforms offer significant potential for advancing tensor network quantum circuit simulation capabilities.

Method: Implemented TN-Sim as a tensor network simulator backend within NWQ-Sim package, utilizing the Tensor Algebra for Many-body Methods (TAMM) framework to support both distributed HPC-scale computations and local simulations with ITensor. Employed task-based parallelization scheme for parallelized gate contraction in wide quantum circuits with many gates per layer. Integrated TAMM framework with Matrix Product State (MPS) tensor network approaches.

Result: Demonstrated an MPS tensor network simulator running on the state-of-the-art Perlmutter (NVIDIA) supercomputer. Achieved scalable simulation environment that can transition from local systems to HPC clusters. Discussed potential portability to other HPC clusters such as Frontier (AMD) and Aurora (Intel).

Conclusion: TN-Sim provides a scalable tensor network simulation framework for quantum circuits that leverages modern HPC architectures. The integration with TAMM enables both local and distributed simulations, with demonstrated performance on leading supercomputers. Future improvements include support for different tensor network topologies and enhanced computational efficiency.
...
TL;DR: Hybrid classical-quantum algorithm H-DES solves nonlinear differential equations on IBM's 156-qubit quantum computers, demonstrating progress for NISQ-era simulations.


<details>
  <summary>Details</summary>
Motivation: To advance physically relevant simulations on current Noisy Intermediate-Scale Quantum (NISQ) devices by solving nonlinear differential equations that are challenging for classical computers.

Method: Developed and implemented the H-DES (Hybrid Differential Equation Solver) algorithm on IBM's quantum platform, specifically testing on 156-qubit quantum computers to solve one-dimensional material deformation problems and the inviscid Burgers' equation.

Result: Successfully demonstrated that the hybrid classical-quantum algorithm H-DES can solve nonlinear differential equations, including one-dimensional material deformation and inviscid Burgers' equation, on current quantum hardware.

Conclusion: This work represents a significant step toward performing physically relevant simulations on present-day NISQ devices, showing that hybrid quantum-classical approaches can solve nonlinear differential equations on current quantum hardware.
...
TL;DR: Nanowire cavity design using quasi-bound states in continuum achieves high Purcell enhancement (∼17) with 4nm bandwidth, enabling efficient directional emission for quantum dot entangled photon sources.


<details>
  <summary>Details</summary>
Motivation: Quantum dot-based entangled photon sources in nanowire waveguides suffer from poor single-photon indistinguishability, limiting their applicability in quantum networks. Existing approaches struggle to combine high Purcell enhancement with the advantages of nanowires (directional emission, broad bandwidth, high extraction efficiency).

Method: Proposed a nanowire cavity design based on quasi-bound states in the continuum formed by strong coupling of two resonant optical modes. Numerical simulations were used to predict cavity performance characteristics.

Result: Numerical predictions show the cavity supports a mode with 4 nm bandwidth and Purcell enhancement of ∼17. The design achieves directional far-field emission (88% overlap with Gaussian) with light extraction efficiency of ∼74%.

Conclusion: The proposed nanowire cavity design enables generation of entangled photon pairs with enhanced extraction efficiency and single-photon indistinguishability, providing a practical route toward realizing quantum networks.
...
TL;DR: Algorithm for pure state tomography using only single-qubit Pauli measurements achieves near-optimal copy complexity of $\widetilde{O}(2^n/ε)$, improving previous $\widetilde{O}(3^n/ε)$ bound.


<details>
  <summary>Details</summary>
Motivation: Pure state tomography typically requires complex measurements or has suboptimal copy complexity. Existing methods for pure state tomography either use complicated measurements or have higher copy complexity. The goal is to develop an algorithm that uses only simple, nonadaptive single-qubit Pauli measurements while achieving near-optimal copy efficiency.

Method: The algorithm uses only nonadaptive Pauli measurements (single-qubit measurements) on $\widetilde{O}(2^n/ε)$ copies of an unknown pure $n$-qubit state. It runs in polynomial time in $2^n$ and $1/ε$, and outputs an estimate state with high fidelity to the true state.

Result: Achieves copy complexity of $\widetilde{O}(2^n/ε)$, which is near-optimal, improving upon the previous best bound of $\widetilde{O}(3^n/ε)$. The algorithm uses only simple single-qubit Pauli measurements, runs efficiently, and outputs a state with fidelity $1-ε$ with high probability.

Conclusion: This work demonstrates that near-optimal pure state tomography can be achieved using only simple, nonadaptive single-qubit measurements, significantly improving the practical feasibility of quantum state tomography while maintaining theoretical optimality.
...
TL;DR: The paper analyzes pentagon holographic quantum error correcting codes using ZX-calculus, studies their properties diagrammatically, and introduces new codes from dual hyperbolic tessellations.


<details>
  <summary>Details</summary>
Motivation: To provide a diagrammatic understanding of pentagon holographic quantum error correcting codes using ZX-calculus framework, enabling better analysis of their properties and facilitating construction of new codes.

Method: Express pentagon holographic code tensors as ZX-diagrams, analyze stabilizer structure via Pauli webs, study logical operators, encoding isometries, Rényi entropy, and black hole/wormhole toy models diagrammatically. Construct new code family from ZX-diagrams on dual hyperbolic tessellations.

Result: Obtained diagrammatic understanding of pentagon holographic code properties. Introduced new code family from dual hyperbolic tessellations and analyzed their logical error rates using belief propagation decoders.

Conclusion: ZX-calculus provides powerful framework for analyzing holographic quantum codes, enabling diagrammatic insights into their structure and facilitating construction of new codes with analyzable error rates.
...
Method: Introduce momentum-space entanglement entropy and analytically prove its behavior in transverse-field Ising model and Su-Schrieffer-Heeger chain, showing saturation to maximal value at critical momenta associated with DQPTs.

Result: Every critical momentum associated with a DQPT saturates its entanglement entropy to maximal value ln(d) (d=2 in TFI and SSH models), coinciding with vanishing Loschmidt echo, providing universal direct signature of DQPTs.

Conclusion: Saturation of mode entanglement provides universal signature of dynamical quantum phase transitions, establishing unified entanglement-based perspective on DQPTs.
...
<details>
  <summary>Details</summary>
Motivation: Single-photon detectors create bottlenecks in QKD systems due to limited detection rates, low efficiency, and high dead-time, which restrict secret key rates. There's a need to optimize QKD protocols for continuous operation in real-world networks and enable multi-user transmission.

Method: 1) Combined time-bin information from two detectors on the receiver's data line for COW QKD protocol; 2) Implemented point-to-multipoint COW QKD with additional receiver module; 3) Used post-processing with OTP encryption for three users (one transmitter, two receivers); 4) Optimized experimental parameters within security margins.

Result: Increased secret key rates with minimal increase in QBER by combining detector time-bin information. Dual-receiver extension improved combined secret key rates of the system when experimental parameters were properly optimized.

Conclusion: The methods successfully overcome detector bottlenecks in COW QKD, enabling higher secret key rates and multi-user transmission. These techniques are generalizable to any COW protocol implementation and support practical QKD deployment in real-world networks.
...
TL;DR: Raman spectroscopy reveals a unique ΔJ=0 rotational excitation in dense hydrogen isotopes that becomes non-zero in solids due to crystal field effects, shows isotope independence, and exhibits splitting in deuterium's broken symmetry phase, indicating a fundamentally different excitation type from conventional harmonic oscillators or quantum rotors.


<details>
  <summary>Details</summary>
Motivation: To investigate the unusual behavior of ΔJ=0 rotational excitations in dense hydrogen isotopes (H2, D2, and their mixtures) across different phases and pressures, particularly focusing on their deviation from expected isotope scaling and zero Raman shift behavior.

Method: Raman spectroscopy measurements performed on dense H2, D2, and H2+D2 mixtures across a wide pressure-temperature range, examining the ΔJ=0 rotational excitation in gas/fluid and solid states, including phases I and II.

Result: ΔJ=0 excitation has zero Raman shift in gas/fluid state but becomes non-zero in solids (e.g., 75 cm⁻¹ at ~50 GPa and 10 K) due to crystal field effects. In deuterium, the mode splits in phase II, suggesting complex molecular environment in broken symmetry phase. Most remarkably, the frequencies show complete isotope independence - they don't scale as rotational (factor of 2) or vibrational (square root of 2) modes.

Conclusion: The ΔJ=0 transition represents a fundamentally different type of excitation from conventional harmonic oscillators and quantum rotors, characterized by its unique isotope independence and behavior under crystal field effects in solid phases.
...
TL;DR: Single-spin multimode measurement primitive using dispersive shifts enables efficient characterization of multimode bosonic registers in trapped ions.


<details>
  <summary>Details</summary>
Motivation: Trapped ions naturally host multiple motional modes alongside spin qubits, creating scalable multimode bosonic registers. Efficient characterization requires accessing many motional modes with limited spin resources.
...
Conclusion: The SDR-based Ramsey sequence provides an efficient measurement primitive for characterizing multimode bosonic registers in trapped ion systems, enabling advanced quantum state analysis with limited spin resources.
...
TL;DR: Extension of PILD method to handle time-dependent external fields and Floquet systems without requiring memory kernel evaluation.


<details>
  <summary>Details</summary>
Motivation: The original PILD method was limited to time-translational invariant systems and couldn't account for time-dependent external fields, restricting its applicability to systems with pumps, drains, or Floquet dynamics.

Method: Developed an alternate, simpler formulation of PILD that eliminates the need for evaluating non-Markovian memory kernels directly, enabling treatment of time-dependent Hamiltonians.

Result: Successfully extended PILD to handle time-dependent external fields and Floquet systems, overcoming the original method's limitation to time-translational invariant cases.

Conclusion: The new PILD formulation provides a more versatile framework for studying quantum systems with empirical processes (pumps/drains) under time-dependent external fields, including Floquet systems.
...
TL;DR: Cyclic entanglement configuration of four particles enhances Hardy-type nonlocality compared to fully entangled systems, with more conditions contradicting local hidden variable models, though experimental results on IBM Brisbane show deviations from simulations.


<details>
  <summary>Details</summary>
Motivation: Previous studies of Hardy's nonlocality have focused primarily on fully entangled systems, leaving other entanglement configurations like cyclic arrangements unexplored. The authors aim to investigate Hardy-type nonlocality in a four-particle cyclic entanglement configuration where each particle is entangled only with its two neighbors.

Method: The study examines a four-particle system arranged in a cyclic entanglement configuration where each particle forms entangled pairs with two neighbors while non-neighboring particles remain unentangled. The researchers implement quantum circuits compatible with this cyclic entanglement structure, simulate the correlation patterns and states of interest, and execute the proposed circuits on IBM Brisbane quantum hardware.

Result: The cyclic entanglement structure offers a larger set of conditions that lead to contradiction with local hidden variable (LHV) models compared to fully entangled systems. This enhancement is attributed to multiple excluded states and correlations where measurement results of a particle only influence its paired partners. While simulations identify correlation patterns and states of interest, experimental results on IBM Brisbane show considerable deviations from simulation counterparts.

Conclusion: Cyclic entanglement configurations provide enhanced Hardy-type nonlocality with more conditions contradicting LHV models than fully entangled systems, demonstrating the importance of entanglement structure in nonlocality studies. However, practical quantum hardware implementations face significant challenges in reproducing theoretical predictions due to noise and imperfections.
...
TL;DR: SurgeQ is a hardware-software co-design strategy that accelerates quantum circuit execution and improves fidelity by using faster two-qubit gates with strengthened coupling while mitigating increased crosstalk through tailored scheduling.


<details>
  <summary>Details</summary>
Motivation: Superconducting quantum platforms face a fundamental trade-off between gate errors and crosstalk when executing quantum circuits. Faster gates typically have higher error rates and increased crosstalk, while slower gates reduce crosstalk but increase circuit execution time and decoherence errors.

Method: SurgeQ employs a two-phase approach: 1) Design phase: uses coupling-strengthened, faster two-qubit gates while establishing a systematic evaluation pipeline with composite noise models to identify optimal coupling strength. 2) Execution phase: implements a tailored scheduling strategy to mitigate the increased crosstalk from faster gates.

Result: Evaluations on real-world benchmarks show SurgeQ achieves higher fidelity than state-of-the-art baselines, effectively combats exponential fidelity decay in large-scale circuits, and achieves up to a million-fold improvement in large-scale circuit fidelity.

Conclusion: SurgeQ demonstrates that hardware-software co-design can successfully balance the gate error-crosstalk trade-off in superconducting quantum systems, enabling accelerated circuit execution with improved overall program fidelity through optimized coupling strength and intelligent scheduling.
...
TL;DR: The paper introduces a novel approach to regularizing singular interactions in quantum mechanics and field theory using coherently controlled, postselected superpositions of evolutions, which naturally produces Gaussian energy filters that suppress high-energy contributions while preserving the target unitary dynamics in appropriate limits.


<details>
  <summary>Details</summary>
Motivation: Standard regulators for singular interactions in quantum mechanics and field theory (cutoffs, higher-derivative terms, heat-kernel smearing, lattice discretizations) impose suppressions by hand. The authors seek a more natural approach where smooth filters arise from physical interference effects in coherently controlled quantum systems.

Method: The method uses coherently controlled, postselected superpositions of evolutions. A successful postselection implements a single heralded operator that is a coherent linear combination of time-evolution operators. For a Gaussian superposition of time translations in quantum mechanics, this yields $V_{σ,Δt}=e^{-iHΔt}\,e^{-\frac12σ^2Δt^2H^2}$, combining the desired unitary step with a Gaussian energy filter. In scalar QFT, this approach induces local Gaussian smearing of interactions, producing symmetry-compatible large-field stabilizers.

Result: The Gaussian energy filter suppresses energies above order $1/(σΔt)$, making short-time kernels in time-sliced path-integral approximations well behaved for singular potentials. The target unitary dynamics is recovered as $σ\to0$ and as $Δt\to0$ at fixed $t$. In scalar QFT, the method induces a positive $(σ^2/2)φ^8$ term in the Euclidean action, providing a symmetry-compatible large-field stabilizer that can be renormalized at fixed $σ$ and removed by taking $σ\to0$. The paper provides short-time error bounds and analyzes multi-step success probabilities.

Conclusion: The paper demonstrates that smooth regularization filters can arise naturally from interference in coherently controlled quantum systems rather than being imposed by hand. This approach provides a physically motivated regularization scheme that preserves symmetries and can be systematically removed to recover the target dynamics, offering new perspectives on regularization in quantum mechanics and field theory.
...
TL;DR: Hybrid quantum-classical neural networks show comparable performance to classical models in best cases but generally degrade with quantum components, requiring cautious design choices.
...
Method: Rigorous statistical study systematically assessing common hybrid models on medical signal data, planar and volumetric images, examining influence of classical and quantum aspects including encoding schemes, entanglement, and circuit size.
...
TL;DR: First demonstration of high-cooperativity coupling of silicon-vacancy centers to hybrid-integrated GaP nanophotonic cavities on diamond, enabling spin-dependent transmission switching and single-shot spin readout.


<details>
  <summary>Details</summary>
Motivation: Current quantum defect platforms using diamond nanophotonic cavities face scalability limitations due to suspended geometries and weak nonlinearity, requiring coupling to secondary processing chips. A scalable, planar platform is needed for quantum networking applications.

Method: Integrated over 600 gallium phosphide (GaP) nanophotonic cavities on a diamond substrate with near-surface silicon-vacancy (SiV) centers. Examined a device with two strongly coupled SiV centers, confirming above-unity cooperativity through multiple independent measurements. Applied external magnetic field via permanent magnet for optical resolution of spin transitions.

Result: Achieved high-cooperativity coupling (C>1) of quantum defects to hybrid-integrated nanophotonics. Determined spin-relaxation time T₁>0.4 ms at 4 K. Observed spin-dependent transmission switching and quantum jumps of SiV spin via single-shot readout.

Conclusion: GaP-on-diamond platform establishes a scalable planar architecture for quantum network applications, combining high cooperativity coupling with GaP's strong nonlinear properties for integrated quantum photonics.
...
TL;DR: Analytical bound states derived for diatomic molecule potentials (Yukawa + four-parameter) using path integral formalism, with thermodynamic properties obtained from compact energy equation.


<details>
  <summary>Details</summary>
Motivation: To investigate approximate analytical bound states for diatomic molecules using a combination of Yukawa and four-parameter potentials within the path integral framework, enabling derivation of energy spectra, wave functions, and thermodynamic properties.

Method: Path integral formalism with appropriate approximation for centrifugal term; energy spectrum and normalized wave functions derived from poles of Green's function and its residues; partition function and thermodynamic properties obtained from compact energy equation.

Result: Derived analytical expressions for bound state energy spectrum and normalized wave functions; obtained partition function and other thermodynamic properties using the compact energy equation formulation.

Conclusion: Successfully developed approximate analytical solutions for bound states of diatomic molecule potentials using path integral methods, providing a framework for calculating both quantum mechanical and thermodynamic properties.
...
TL;DR: Framework for quantum simulation of thermal states with synthetic interactions using unitary dynamics, applicable to digital/analog devices, with resource requirements independent of temperature and criticality.


<details>
  <summary>Details</summary>
Motivation: Most quantum simulation work focuses on either ground states with effective interactions or thermal states with native interactions, leaving a gap for simulating thermal states with respect to synthetic interactions that could answer many open questions.

Method: Unitary dynamics-based framework for designing quantum simulations of thermal states with non-native interactions; uses classical means to find control sequences for target thermal states; demonstrated with cluster Ising model containing three-body interactions.

Result: Method enables simulation of thermal states for system sizes beyond state-vector/density-matrix classical methods; required experimental resources (total evolution time) are independent of temperature and criticality.

Conclusion: Provides practical framework for quantum simulation of thermal states with synthetic interactions on current quantum devices, overcoming limitations of existing approaches and enabling exploration of previously inaccessible thermal physics questions.
...
TL;DR: Quantum Wiener (qWiener) architectures combine quantum linear dynamic networks with weak continuous measurements and classical nonlinear readouts, retaining fading-memory and universality properties while offering performance gains over classical and quantum reservoir computing models.


<details>
  <summary>Details</summary>
Motivation: To develop a rigorous theoretical foundation for quantum reservoir computing, specifically quantum Wiener architectures, by establishing their fundamental properties and demonstrating their advantages over existing classical and quantum approaches.

Method: The paper introduces quantum Wiener (qWiener) architectures consisting of quantum linear dynamic networks with weak continuous measurements and classical nonlinear static readouts. It provides rigorous proofs of fading-memory property and universality, develops a kernel-theoretic interpretation showing qWiener reservoirs induce deep kernels, characterizes the simplest instantiation using concatenated quantum harmonic oscillators, and empirically evaluates the architecture on standard reservoir computing benchmarks.

Result: The paper proves that qWiener systems retain fading-memory property and universality despite quantum constraints. It shows qWiener reservoirs naturally induce deep kernels, characterizes differences from classical Wiener systems, and demonstrates systematic performance gains over prior classical and quantum reservoir computing models in empirical evaluations.

Conclusion: Quantum Wiener architectures provide a theoretically sound and practically effective framework for quantum reservoir computing, combining the advantages of quantum dynamics with classical nonlinear readouts while maintaining fundamental computational properties and offering improved performance over existing approaches.
...
TL;DR: PACOX is the first FPGA-based accelerator for Pauli string computation that achieves 100× speedup over CPU methods with superior energy efficiency for hybrid quantum-classical systems.


<details>
  <summary>Details</summary>
Motivation: Classical computation of Pauli strings suffers from exponential complexity and becomes a performance bottleneck as qubit count increases in hybrid quantum-classical algorithms.

Method: Proposes PACOX accelerator using compact binary encoding with XOR-based index permutation and phase accumulation, implemented as parallel and pipelined processing element cluster architecture on FPGA.

Result: PACOX operates at 250 MHz with 0.33W power, using 8,052 LUTs, 10,934 FFs, and 324 BRAMs. Achieves up to 100× speedup over CPU methods for Pauli strings up to 19 qubits with lower memory usage and power-delay product.

Conclusion: PACOX delivers high computational speed with superior energy efficiency for Pauli-based workloads, demonstrating FPGA acceleration as an effective solution for hybrid quantum-classical systems.
...
TL;DR: Noise Tailoring (NT) modifies two-qubit gate noise via statistical sampling to enhance error mitigation effectiveness, achieving up to 5x accuracy improvement in simulations but facing limitations on real hardware due to non-Markovian noise.


<details>
  <summary>Details</summary>
Motivation: Error mitigation methods are essential for NISQ quantum computers but are often optimized for specific noise types that may not match actual hardware noise, creating a need for noise adaptation strategies.

Method: Noise Tailoring (NT) protocol that statistically samples and modifies the structure of two-qubit gate noise to make it more amenable to existing error mitigation techniques.

Result: Classical emulation shows NT+EM can be up to 5 times more accurate than EM alone for realistic Pauli noise on two-qubit gates, but real IBM quantum hardware performance is limited by non-Markovian noise sources.

Conclusion: NT is effective for improving error mitigation in simulated environments with Pauli noise but reveals hardware-specific noise characteristics on real quantum computers, suggesting its utility for hardware characterization and development guidance.
...
TL;DR: Demonstration of an unconditional remote CNOT gate between diamond-based qubit devices using NV centers and nuclear spins, enabling distributed quantum computing without post-selection.
...
Method: Used diamond-based qubit devices with Carbon-13 nuclear spins as control/target qubits and NV electron spins for local logic, readout, and remote entanglement generation. Implemented deterministic logic, single-shot readout, and real-time feed-forward without post-selection.

Result: Successfully demonstrated an unconditional remote CNOT gate and created a Greenberger-Horne-Zeilinger state showing genuine 4-partite entanglement shared between nodes, benchmarking the system's capabilities.

Conclusion: The results demonstrate a key capability for solid-state quantum networks, enabling exploration of distributed quantum computing and testing of complex network protocols on fully integrated systems.
...
TL;DR: The paper investigates how coherent and incoherent errors in quantum chaotic systems lead to distinct signatures in time-reversed dynamics, showing linear error accumulation for incoherent errors versus quadratic-to-linear crossover for coherent errors.


<details>
  <summary>Details</summary>
Motivation: To understand how coherent errors (from imperfect Hamiltonian control) and incoherent errors (from environmental coupling) imprint distinct signatures on the emergent irreversibility of many-body dynamics in quantum chaotic systems, where both types of errors are exponentially amplified due to information scrambling.

Method: The authors investigate multi-round time-reversed dynamics using scramblon theory to obtain closed-form expressions for the Loschmidt echo over different rounds of time-reversed evolution, and explicitly verify predictions using the solvable Sachdev-Ye-Kitaev (SYK) model.

Result: For incoherent errors, error accumulation is linear with the number of rounds, while coherent errors exhibit a crossover from quadratic to linear accumulation. These predictions are verified using the SYK model.

Conclusion: The results provide a theoretical foundation for characterizing and calibrating coherent and incoherent errors in reversed dynamics, with particular relevance to nuclear magnetic resonance systems.
...
TL;DR: The paper provides an intuitive construction of Mackey's quantum logical axiomatic system and proves that composed quantum systems must be described via tensor product spaces, building on Aerts and Daubechies' work.


<details>
  <summary>Details</summary>
Motivation: To provide a detailed discussion and extension of Aerts and Daubechies' work on justifying the tensor product for describing joint quantum systems, showing how composed physical systems from classical and quantum mechanics should be described logically.

Method: Uses Mackey's quantum logical axiomatic system construction, discusses a special class of axiomatically defined composed physical systems, and employs results from lattice theory and c-morphism theory to provide detailed proofs.

Result: Presents a detailed proof that in quantum mechanics, a composed physical system must be described via a tensor product space, extending the results from Aerts and Daubechies' paper.

Conclusion: The work successfully demonstrates the logical necessity of using tensor product spaces for describing composed quantum systems within Mackey's axiomatic framework, providing rigorous mathematical justification for this fundamental quantum mechanical construction.
...
TL;DR: Virtual temperatures of passive quantum states are analyzed through majorization theory, revealing their role in determining heat flow direction and bounding quantum Otto engine efficiency via min-max virtual temperatures.


<details>
  <summary>Details</summary>
Motivation: To establish virtual temperature as a key operational quantity linking passivity and majorization to optimal performance of quantum thermal machines, drawing parallels between quantum thermodynamic processes and classical counterparts.

Method: Apply majorization theory to analyze virtual temperatures of passive quantum states; define mean temperature over virtual temperatures of adjacent energy levels; characterize intermediate passive states in quantum Otto engine; derive efficiency bound using min-max virtual temperatures; demonstrate with coupled-spins system example.

Result: Virtual temperatures determine heat flow direction via majorization relations; min-max virtual temperatures provide upper bound for Otto engine efficiency; coupled-spins system serves as explicit demonstration; virtual temperatures enable quantum-classical thermodynamic parallels.

Conclusion: Virtual temperature emerges as fundamental operational quantity connecting passivity, majorization, and optimal performance of quantum thermal machines, providing theoretical framework for analyzing quantum thermodynamic processes.
...
TL;DR: A free-running RFI-MDI-QKD protocol using classification-distillation achieves 9x higher key rates and >24 dB channel loss tolerance under rapid reference-frame variations, enabling practical mobile QKD applications.


<details>
  <summary>Details</summary>
Motivation: Existing RFI-MDI-QKD implementations assume fixed or slowly drifting reference-frame misalignment, which is unrealistic outside laboratory settings. Rapid, free-running reference-frame variations in real-world environments severely degrade key rates and transmission distances of conventional RFI-MDI-QKD protocols.

Method: Proposes a free-running RFI-MDI-QKD protocol that introduces a classification-distillation method to reclassify total detection events. This approach extracts secure keys without modifying experimental setups, maintaining high-rate key generation under rapid reference-frame variations.

Result: Achieves key rates more than nine times higher than the best previous RFI-MDI-QKD scheme and tolerates channel losses exceeding 24 dB, where earlier approaches fail. Enables practical quantum key distribution on mobile platforms including satellite-to-ground links and airborne nodes.

Conclusion: The proposed free-running RFI-MDI-QKD protocol with classification-distillation method overcomes limitations of conventional approaches under rapid reference-frame variations, enabling practical deployment of QKD on mobile platforms with significantly improved performance metrics.
...
TL;DR: Junction readout architecture achieves fast, high-fidelity qubit measurement using hybrid capacitive-Josephson coupling, enabling bifurcation-based readout without Purcell filters or quantum-limited amplifiers.


<details>
  <summary>Details</summary>
Motivation: Dispersive readout suffers from Purcell decay and measurement-induced state transitions, limiting speed and fidelity compared to quantum gates. Current approaches require Purcell filters and near-quantum-limited amplifiers, adding hardware complexity.

Method: Junction readout couples transmon qubit to readout resonator through both capacitance and Josephson junction, creating strong cross-Kerr interaction without transverse coupling. This enables intrinsic Purcell protection and enhanced resilience to MIST. The nonlinear coupling is exploited to engineer large Kerr nonlinearity for bifurcation-based readout.

Result: Achieved 99.4% assignment fidelity with 68 ns integration time and 98.4% QND fidelity without external Purcell filter or near-quantum-limited amplifier. Demonstrated intrinsic Purcell protection and enhanced resilience to measurement-induced state transitions.

Conclusion: Junction readout with bifurcation-based readout provides scalable, practical alternative to dispersive readout, enabling fast, high-fidelity qubit measurement with reduced hardware overhead.
...
<details>
  <summary>Details</summary>
Motivation: Efficient estimation of quantum coherence and entanglement for unknown states remains challenging in high-dimensional systems, with quantum state tomography being resource-intensive. Need for practical methods using minimal experimental resources.

Method: Support vector regression (SVR) approach requiring only diagonal entries of density matrix plus traces of squared and cubed density matrices for coherence estimation; for entanglement, additionally requires traces of squared and cubed reduced density matrices. Support vector quantile regression (SVQR) with pinball loss prevents overestimation.

Result: Method significantly reduces resource overhead compared to quantum state tomography while maintaining high accuracy. SVQR ensures over 95% of predictions are conservative lower bounds in most cases, with over 93% lower-bound reliability despite 2% perturbations in input features.

Conclusion: The technique provides a practical and scalable tool for characterizing quantum resources across computation, communication, and metrology applications, enabling efficient estimation with minimal experimental resources.
...
TL;DR: Chiral quantum-dot assemblies enable room-temperature observation of quantum coherent spin dynamics in chiral-induced spin selectivity (CISS) effects.


<details>
  <summary>Details</summary>
Motivation: While spin selectivity in chiral molecular systems (CISS) has been widely studied, its quantum coherence properties remain unexplored, creating a gap in understanding the fundamental quantum aspects of this phenomenon.

Method: Investigating spin-dependent photoluminescence dynamics in multilayer quantum-dot assemblies coupled by chiral linkers using circularly polarized excitation with external magnetic fields, analyzing PL lifetime modulation based on magnetic field magnitude and geometry.

Result: Observation of pronounced PL lifetime modulation dependent on magnetic field parameters, with lifetime differences between left- and right-circularly polarized excitations showing field-angle dependence consistent with spin precession driven by transverse magnetic-field components relative to chiral axis.

Conclusion: Chiral QD assemblies provide a room-temperature platform for probing quantum coherent manifestations of CISS effects, with significant implications for spintronic and quantum technologies.
...
TL;DR: QNNs maintain near-perfect accuracy with 6-bit DACs, but training requires special techniques below 12-bit resolution due to gradient deadlock.


<details>
  <summary>Details</summary>
Motivation: Quantum computer scaling requires cryogenic control electronics with severe power/area constraints, necessitating investigation of how low-resolution DACs affect QNN training and inference.

Method: Investigated QNN training/inference under finite DAC resolution constraints; introduced temperature-controlled stochasticity with probabilistic parameter updates to overcome gradient deadlock at low resolutions.

Result: Pre-trained QNNs achieve near-infinite-precision accuracy with 6-bit DACs (diminishing returns beyond 4 bits); training requires temperature-controlled stochasticity below 12-bit resolution, enabling successful training at 4-10 bits matching/exceeding baseline performance.
...
TL;DR: Protocol for quantum-circuit thermalization using engineered reservoir qubits enables non-uniform heating and amplification-dissipation dynamics for out-of-equilibrium states.


<details>
  <summary>Details</summary>
Motivation: To develop a new approach for realizing out-of-equilibrium quantum states at given temperatures that overcomes limitations of conventional thermal reservoirs.

Method: Quantum-circuit platform coupling system with engineered reservoir qubits, leveraging non-orthogonality of qubit eigenstates to enable non-uniform heating via modified Kubo-Martin-Schwinger relation and amplification-dissipation dynamics through violation of microscopic time-reversibility.

Result: Demonstrated two applications: temporally-correlated dichromatic emission and Liouvillian exceptional point protected quantum synchronization at finite temperatures, both challenging with conventional thermal reservoirs.

Conclusion: The protocol provides a novel framework for implementing complex-balanced thermalization via Markovian processes on quantum circuits, enabling previously challenging out-of-equilibrium phenomena at finite temperatures.
...
TL;DR: Frequency-ramped microwave pulses using FPGA modulation achieve 4x better spin population transfer than resonant excitation for boron vacancy defects in hBN, enabling 16x faster quantum sensing.


<details>
  <summary>Details</summary>
Motivation: Boron vacancies in hBN are promising quantum sensors but suffer from broad hyperfine-split spin transitions that limit spectral coverage for conventional resonant excitation, even with isotopic enrichment.

Method: Implemented frequency-ramped microwave pulses via FPGA-based frequency modulation, analyzed using quantum dynamics simulations with an effective two-state Landau-Zener model incorporating relaxations.

Result: Frequency-ramped pulses achieved ~4x greater spin-state population transfer (|0⟩→|-1⟩) and contrast than resonant microwave excitation, enabling 16x shorter measurement time for spin relaxation-based quantum sensing.

Conclusion: The FPGA-implemented frequency-ramped microwave approach is robust and valuable for quantum relaxometry with spin defects in hBN, particularly in noisy environments.
...
TL;DR: Quantum generative adversarial networks (QGANs) show exponential advantage in capacity scaling for image generation when using hybrid latent style-based architecture with quantum generators, requiring far fewer trainable parameters than classical counterparts.


<details>
  <summary>Details</summary>
Motivation: While quantum generative adversarial networks (QGANs) have shown promise in various applications and hint at using far fewer trainable parameters than classical counterparts for comparable performance, this advantage has never been systematically studied or quantified.

Method: The study uses a hybrid latent style-based QGAN architecture with a classical variational autoencoder to encode input data into latent space, followed by a style-based quantum generator for data generation. The research focuses on SAT4 image generation with careful tuning of the autoencoder to obtain stable, reliable results.

Result: The optimal capacity (number of trainable parameters) of both the classical discriminator and classical generator scales exponentially with respect to the capacity of the quantum generator. This demonstrates an exponential advantage in capacity scaling for quantum generators in the hybrid architecture.

Conclusion: The study provides the first comprehensive experimental evidence of quantum advantage in generative modeling, showing that quantum generators require exponentially fewer parameters than classical counterparts to achieve comparable performance in image generation tasks.
...
TL;DR: The paper demonstrates that the Mpemba effect (hotter states relaxing faster) can be exploited for enhanced quantum thermometry, enabling nonequilibrium probes to transiently outperform equilibrium strategies in temperature estimation.


<details>
  <summary>Details</summary>
Motivation: Traditional quantum thermometry relies on equilibrium probes, which imposes limitations on sensitivity tied to thermalization times and cannot be improved in fast, noisy, or nonstationary settings. There's a need to overcome these limitations for nanoscale devices and quantum technologies.

Method: The authors prove that Mpemba-type inversions generically enhance quantum Fisher information (QFI) for temperature estimation. They analyze two-level and Λ-level probes coupled to bosonic baths to demonstrate that nonequilibrium initializations can transiently outperform equilibrium strategies.

Result: Nonequilibrium initializations can transiently outperform both equilibrium strategies and colder states, realizing a "metrological Mpemba effect." This effect provides finite-time enhancement of QFI for temperature estimation, converting anomalous relaxation into a metrological resource.
...
TL;DR: Quantum algorithm for extracting smooth positive functions from quantum state amplitudes with Heisenberg-limit scaling, improving upon previous methods in dimensionality, qubit efficiency, and preconditioning.


<details>
  <summary>Details</summary>
Motivation: Quantum computers can solve PDEs efficiently but often only prepare solution states; naive extraction via amplitude estimation diminishes speed-up advantages. Need efficient method to extract explicit function representations from quantum state amplitudes.

Method: Technique samples cumulative distribution of encoded function, fits with Chebyshev polynomials, and extracts full function representation. Uses preconditioning to remove dependency on function minimum, reduces quantum complexity with respect to qubit count, and handles higher-dimensional functions.

Result: Achieves Heisenberg-limit scaling for extracting smooth positive functions from quantum state amplitudes. Improves upon previous methods in dimensionality, qubit efficiency, and preconditioning. Validated through small-scale numerical simulations.

Conclusion: Proposed technique enables efficient extraction of explicit function representations from quantum state amplitudes, preserving quantum speed-up advantages for PDE solving and other applications requiring function extraction from quantum states.
...
TL;DR: A unitary, scalable encoding scheme for preparing Pauli eigenstates in surface codes that preserves fault-distance protection, uses only local gates, scales as O(d) depth, and outperforms measurement-based approaches under depolarizing noise.


<details>
  <summary>Details</summary>
Motivation: State preparation is critical in fault-tolerant quantum computation, but existing methods face challenges. Unitary approaches often fail to preserve fault-distance protection as code distance increases, while measurement-based schemes can be costly on platforms where measurements are expensive relative to gates.

Method: Generalizes reinforcement-learned strategies from surface-17 code to arbitrary distances for both rotated and unrotated surface codes. Uses only geometrically local gates compatible with planar 2D connectivity. Designs explicit stabilizer-expanding circuits with and without ancilla-mediated connectivity, achieving O(d) circuit depth consistent with entanglement-generation bounds.

Result: Numerical simulations under depolarizing noise show unitary encoding without ancillas outperforms standard stabilizer-measurement-based schemes, reducing logical error rates by up to an order of magnitude. The scheme preserves fault-distance protection during state preparation.

Conclusion: The work bridges measurement-based and unitary encodings for surface-code states, offering a distance-preserving state preparation method particularly suitable for platforms like trapped ions and neutral atoms where measurements are costly relative to gates.
...
TL;DR: A Kerr-engineered multi-lens protocol generates giant Fock states with over 10,000 photons using quantum-optical duality, achieving high fidelity and inverse-root-N scaling time.


<details>
  <summary>Details</summary>
Motivation: Scalable preparation of bosonic quantum states with macroscopic excitations is fundamentally limited by control complexity and photon loss, restricting prior efforts to only dozens of excitations per mode.

Method: Based on duality between quantum state evolution in Fock space and optical wave-function propagation in waveguide arrays, the protocol uses Kerr-engineered multi-lens operations in a single bosonic mode with optimized phase and displacement operations across lens groups to compensate for non-paraxial aberrations.

Result: Deterministic generation of Fock states exceeding 10,000 photons with fidelities above 73% in numerical simulations for photon numbers up to N=100,000, with execution time scaling as N^{-1/2} and robustness against photon loss.
...
TL;DR: The paper introduces a unified frame theory framework for classical simulation of quantum circuits, providing a common quantitative measure (one-norm of quasi-probability distributions) to compare simulation strategies and discover new algorithms.


<details>
  <summary>Details</summary>
Motivation: To understand the precise boundaries between classical simulability and quantum advantage for noisy quantum circuits, and to develop a unified theoretical framework that encompasses existing simulation strategies while enabling discovery of new approaches.

Method: Develops a frame theory framework for classical simulation, where computational cost is determined by the one-norm of associated quasi-probability distributions. Uses convex optimization tools to explore different frame choices, enabling analysis of existing methods (stabilizer state simulation, Pauli back-propagation) and discovery of new approaches based on generalized Pauli frames.

Result: Provides new insights and improved bounds for existing simulation methods, discovers a new simulation approach with improved performance based on generalized Pauli frames, and demonstrates that classical simulation can benefit from frame theory perspectives beyond traditional quantum resource classifications.

Conclusion: Frame theory provides a powerful unified framework for analyzing and developing classical simulation algorithms for quantum circuits, offering a common quantitative measure across different approaches and enabling discovery of improved simulation strategies that transcend traditional quantum resource classifications.
...
TL;DR: The paper extends simultaneous purification results from bipartite to multipartite communication scenarios, showing that compositions of non-signalling assemblages cannot exceed standard Bell correlations, requiring signalling assemblages for correlations beyond this set.


<details>
  <summary>Details</summary>
Motivation: To extend the simultaneous purification framework from bipartite to multipartite communication scenarios, as many quantum features are genuinely multipartite and cannot be reduced to two-party behavior.

Method: Unify and extend simultaneous purification results from states to instruments and super-instruments (composable structures), then analyze arbitrary compositions of non-signalling assemblages in multipartite communication schemes.
...
Conclusion: The simultaneous purification framework successfully extends to multipartite scenarios, establishing fundamental limitations on what can be achieved with non-signalling assemblages and clarifying when signalling resources are necessary for correlations beyond standard Bell scenarios.
...
TL;DR: Quantum algorithm enables efficient simulation of macroscopic-scale elastic materials using quantum computers, demonstrating exponential advantage over classical methods for simulating coupled oscillator systems like graphene sheets.


<details>
  <summary>Details</summary>
Motivation: Classical molecular dynamics simulations of materials at atomic resolution on macroscopic scales are computationally infeasible due to memory and runtime constraints, even with simplified elastic network models.

Method: Introduces Quantum Elastic Network Models (QENMs) using Babbush et al.'s quantum algorithm that provides exponential advantage for simulating coupled oscillator systems under specific conditions, applied to planar materials like graphene.

Result: Demonstrates efficient simulation of 2D graphene sheets, analyzing complexity for state preparation, Hamiltonian simulation, and measurement; estimates centimeter-scale atomistic simulation requiring only ~160 logical qubits compared to classical methods needing hundreds of petabytes.

Conclusion: Quantum Elastic Network Models enable practical simulation of macroscopic-scale materials with atomic resolution, offering exponential quantum advantage for materials design applications like heat transfer and structural analysis.
...
TL;DR: Majorana Propagation algorithm efficiently simulates time evolution of observables for sparse quartic Hamiltonians when low-degree approximations exist, with runtime scaling as N^O(log(t/ε)).


<details>
  <summary>Details</summary>
Motivation: Hamiltonian evolution simulation is a promising candidate for quantum advantage, but efficient classical algorithms are only expected in restricted settings. The paper aims to identify such a setting and provide provable guarantees for the Majorana Propagation algorithm.

Method: Majorana Propagation algorithm combines Trotter steps with truncations to find low-degree approximations of time-evolved observables. The method is analyzed for sparse quartic Hamiltonians, with runtime scaling as N^O(log(t/ε)) where N is the number of Majorana modes.

Result: The paper proves that Majorana Propagation efficiently finds low-degree approximations when they exist, and can simulate time dynamics of any sparse quartic Hamiltonian up to time t_max(u) depending on interaction strength u. For small u, t_max(u) → ∞, showing accuracy at all times for quadratic Hamiltonians.

Conclusion: Majorana Propagation provides the first provable guarantee for Hamiltonian evolution simulation in a restricted setting, establishing efficient classical simulation for sparse quartic Hamiltonians when low-degree approximations exist, with particular accuracy for near-quadratic systems.
...
TL;DR: A scalable pulse-level control framework using frequency modulation and dynamical decoupling to suppress XY crosstalk errors in dense superconducting quantum processors.


<details>
  <summary>Details</summary>
Motivation: As superconducting quantum processors scale, unwanted XY interactions between nearby qubits cause crosstalk errors that limit performance. Frequency crowding in large-scale systems restricts achievable frequency separation, making residual XY coupling a key challenge during single-qubit operations.

Method: Proposes a scalable pulse-level control framework incorporating frequency modulation (FM) and dynamical decoupling (DD) to suppress XY crosstalk errors. The framework operates independently of coupling strengths, reduces calibration overhead, and supports multi-qubit connectivity.

Result: Numerical simulations show orders-of-magnitude reductions in infidelity for both idle and single-qubit gates in a two-qubit system. Scalability validated in a five-qubit layout where crosstalk between a central qubit and four neighbors is simultaneously suppressed.

Conclusion: The crosstalk suppression framework provides a practical route toward high-fidelity operation in dense superconducting architectures, addressing a critical challenge in scaling quantum processors.
