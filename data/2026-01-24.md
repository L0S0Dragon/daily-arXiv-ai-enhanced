...
Method: Composite model with spin ensemble probe coupled to ancillary qubit via Heisenberg XXZ interaction; optimized weak probe-ancilla coupling; unconditional measurement on qubit creates two parallel evolution paths; parity detection on qubit or probe system.

Result: Quantum Fisher information shows exact or asymptotic quadratic scaling with spin number N (Heisenberg scaling); phase sensitivity approaches Heisenberg limit; robust to imperfect encoding operator and coupling strength variations.

Conclusion: Unconditional measurement on qubit can efficiently replace GHZ-like states and squeezing Hamiltonians for surpassing standard quantum limit in metrology, offering simpler implementation path to Heisenberg-limited precision.
...
TL;DR: Wavepackets enable energy-resolved transport studies on quantum computers, revealing a finite-size mobility edge in Anderson localization and offering error-mitigated measurements with improved statistical precision.


<details>
  <summary>Details</summary>
Motivation: Existing quantum simulation approaches using computational basis states have poor energy resolution for studying energy-dependent transport phenomena, limiting the ability to probe features like mobility edges in disordered systems.

Method: Use wavepackets as tunable-energy probes with small energy variance; implement error mitigation via maximum-likelihood estimation to infer noiseless distributions; develop quantum algorithm for preparing quasiparticle wavepackets in interacting fermion systems.

Result: Demonstrated energy-dependent localization transition (finite-size mobility edge) in Anderson model on 8x7 lattice using Quantinuum H2-2 quantum computer: low-energy wavepackets remain localized while high-energy ones delocalize; error mitigation reduced statistical uncertainty by factor of 5 compared to post-selection.

Conclusion: Wavepacket-based transport studies with error mitigation enable energy-resolved measurements of quantum phenomena like mobility edges on near-term quantum computers, with modest resource requirements that extend to many-body interacting systems.
...
TL;DR: The paper generalizes a re-optimization technique for deep learning decoders of quantum error-correcting codes from the toric code to arbitrary stabilizer codes, addressing label degeneracy issues and showing improved decoding accuracy.


<details>
  <summary>Details</summary>
Motivation: When applying deep learning to decode quantum error-correcting codes, a key challenge arises from the non-uniqueness between syndrome measurements and corresponding error patterns (label degeneracy). Prior work addressed this for the toric code using symmetry-based re-optimization, but generalization to arbitrary stabilizer codes was needed.

Method: Generalized the toric code re-optimization approach to arbitrary stabilizer codes. Used multilayer perceptrons to approximate continuous functions that complement syndrome measurements of the Color code and Golay code. Performed decoder re-optimization for each code and evaluated geometric/algebraic structures in the continuous function approximations.

Result: For the Color code: ~0.8% improvement in decoding accuracy at 5% physical error rate. For the Golay code: ~0.1% accuracy increase. Generalized continuous function design proved advantageous for learning geometric structure inherent in codes. Faithful approximations of code structure significantly impact re-optimization effectiveness.

Conclusion: The re-optimization technique previously effective for the toric code can be generalized to address label degeneracy challenges when applying deep learning to stabilizer code decoding. Design of generalized continuous functions that capture code structure is crucial for effective re-optimization.
...
TL;DR: Computational quantum capacities show stark separation from unbounded capacities under cryptographic assumptions, with polynomial-complexity channels having zero computational capacity but maximal unbounded capacity.


<details>
  <summary>Details</summary>
Motivation: Traditional quantum channel capacities don't limit computational resources of sender/receiver, but practical quantum communication requires computationally efficient encoding and decoding operations.

Method: Initiate study of computational quantum capacities by focusing on computational two-way quantum capacity, connecting it to computational distillable entanglement of channel's Choi state, and analyzing under standard cryptographic assumptions.

Result: Under cryptographic assumptions, there exists polynomial-complexity quantum channels with zero computational two-way quantum capacity but nearly maximal unbounded capacity, showing sharp transition from maximal to zero capacity when channel complexity leaves polynomial realm.

Conclusion: Computational efficiency requirements radically alter quantum communication limits, demonstrating fundamental separation between computational and unbounded quantum capacities.
...
TL;DR: The paper presents a unified algebraic framework using quadratic functions over abelian groups/Hopf algebras to describe and efficiently solve various quantum models including Clifford circuits, stabilizer codes, free-particle systems, and rotor/GKP codes.
...
Method: Represent quantum objects (states, operators, etc.) as quadratic tensors based on quadratic functions over abelian groups or Hopf algebras. Different degrees of freedom correspond to different algebraic structures: ℤ₂ for qubits, ℤ_d for qudits, ℝ for continuous variables, ℤ and ℝ/ℤ for rotors, and super Hopf algebra ℱ for fermions. Tensor networks of quadratic tensors can be efficiently contracted using operations similar to Schur complement.

Result: Quadratic tensors with n degrees of freedom require only O(n²) coefficients for full specification. The framework naturally handles mixed degrees of freedom (e.g., qudits of different dimensions) and enables efficient tensor network contraction. Generalized stabilizer codes and Clifford gates are defined for arbitrary abelian groups, with extension to i-th order tensors requiring O(nⁱ) coefficients.

Conclusion: Quadratic functions over abelian groups/Hopf algebras provide a unifying algebraic structure for classically simulable quantum models, enabling efficient representation and manipulation while generalizing to higher-order tensors and mixed quantum systems.
...
<details>
  <summary>Details</summary>
Motivation: To understand how network topology shapes quantum dissipative dynamics and quantum coherence, and to develop approaches for controlling coherence through tailored network structures in complex quantum systems.

Method: Two-part approach: 1) Lindblad master equation analysis of small Ising spin networks coupled to dissipative baths, 2) Mean-field approach extension to larger-scale networks to capture dissipative dynamics in complex systems.

Result: Network topology significantly influences quantum dissipative dynamics and coherence. The study reveals sensitivity of quantum coherence to network structure and provides insights into coherent dynamics of entangled states in networks.

Conclusion: Dissipative quantum dynamics are strongly topology-dependent, offering potential for coherence control through network design. Results have broader implications for complex systems including social models, epidemiology, and biological systems.
...
<details>
  <summary>Details</summary>
Motivation: Quantum LDPC codes are crucial for fault-tolerant quantum computing due to their low-weight check measurements, but fundamental questions remain about how check-weight constraints limit achievable code parameters like rate and distance.

Method: Combines analytical proofs with numerical optimization using linear programming techniques to derive upper bounds on code parameters under check-weight constraints, without assuming geometric locality or special graph connectivity.

Result: Proves that stabilizer codes with weight-3 checks cannot have nontrivial distance; establishes tight rate-distance tradeoffs for CSS codes with weight-4 checks and subsystem codes with weight-2 checks; provides numerical upper bounds for finite-size codes and identifies constructions approaching these limits.

Conclusion: The work provides fundamental limitations on qLDPC code parameters under check-weight constraints, delineating the landscape of practically relevant codes for near-term quantum hardware with tens to hundreds of physical qubits.
...
TL;DR: Quantum algorithm for Fokker-Planck equation achieves exponential speedup in particle number, quartic speedup in error, and quadratic speedup in time compared to classical worst-case bounds for rare event simulations.
...
Result: For η pairwise interacting particles with N plane waves per degree, reactive flux estimated to error ε using Õ((η^{5/2}√(tβ)α_V + η^{3/2}√(t/β)N)/ε) quantum gates. Shows exponential separation in η, quartic speedup in ε, and quadratic speedup in t compared to classical worst-case O(te^{Ω(η)}/ε^4) bounds.

Conclusion: Demonstrates rigorous route toward quantum advantage for high-dimensional dissipative dynamics, overcoming exponential decay issues in previous quantum algorithms for non-unitary Fokker-Planck simulations.
...
TL;DR: Quantum scrambling enables SWAP gates between collective degrees of freedom without local control by combining Hayden-Preskill recovery with quantum teleportation in parallel opposite directions.


<details>
  <summary>Details</summary>
Motivation: To enable generic SWAP gates between collective degrees of freedom in systems lacking universal local control, leveraging quantum information scrambling phenomena.
...
Result: Protocol enables bidirectional exchange of quantum states through global interactions alone, cleanly distinguishing roles of information spreading, entanglement, and chaos for coherent state transfer and recovery.

Conclusion: Quantum scrambling enables practical SWAP gates without local control, with experimental realization proposed using Dicke model in cavity-QED and trapped-ion platforms, highlighting holography's utility in quantum gate design.
...
<details>
  <summary>Details</summary>
Motivation: Silicon vacancy centers in 4H-SiC offer wafer-scale materials maturity with long spin coherence and chip-level photonics, making them promising for scalable quantum technologies. The S=3/2 ground state provides a native qudit for compact encodings but introduces spectator transitions where short, detuned pulses can coherently drive non-addressed level pairs, creating crosstalk that needs characterization.

Method: Used broadband Ramsey interferometry to reveal and quantify spectator-transition crosstalk. Analytically mapped spectral lines to pairwise energy differences between qudit levels of the rotating-frame Hamiltonian, assigning weights via compact amplitudes from prepared state and microwave pulse parameters. Numerically propagated time-domain simulations with experimental sampling to reproduce detuning maps.

Result: Ramsey Fourier spectra displayed multiple lines beyond addressed single-quantum transitions. Analytic predictions revealed deterministic six-branch structure. Measured peak positions coincided with analytic branch lines without frequency fitting. Numerical simulations reproduced experimental detuning maps.

Conclusion: Provides a practical, spectator-aware framework for multilevel control in silicon vacancy qudits. Offers guidance to suppress crosstalk or exploit spectator lines for in situ pulse calibration and phase-sensitive quantum state/process estimation.
...
<details>
  <summary>Details</summary>
Motivation: Squeezed states of light enable enhanced measurement precision below the standard quantum limit, but generating and detecting bright pulsed squeezing at high levels for nonlinear microscopy applications remains challenging due to photodamage and quantum-limited noise constraints.

Method: Used χ² optical parametric amplification process in a waveguide to generate bright picosecond pulsed squeezed light, with measurement of both bright squeezing and vacuum squeezing, corrected for losses to determine actual squeezing levels generated in the waveguide.
...
Conclusion: The demonstrated efficient technique for generating high-level bright pulsed squeezed light will facilitate broader adoption of quantum-enhanced nonlinear microscopy in biological studies by overcoming previous limitations in squeezing generation and detection.
...
TL;DR: Quantum optimal control using adaptive sliced GRAPE pulses enhances atom interferometer mirror pulse robustness to experimental inhomogeneities while maintaining high efficiency.


<details>
  <summary>Details</summary>
Motivation: Atom interferometers require high-efficiency mirror pulses that perform robustly under experimental inhomogeneities like detuning variations, Rabi frequency fluctuations, response-time delays, coupling inhomogeneity, and velocity spread.

Method: Used gradient ascent pulse engineering (GRAPE) to design optimized mirror pulses for Mach-Zehnder light-pulse atom interferometers by discretizing control into non-uniform phase slices (adaptive sliced structure).

Result: Optimized pulses showed broad tolerance to detuning variations [-Ω₀,Ω₀] and Rabi frequency variations [0.1×Ω₀,1.9×Ω₀] (Ω₀=2π×25 kHz), maintained high transfer efficiency with response-time delays up to 1.6 μs, and were robust to coupling inhomogeneity and velocity spread.
...
TL;DR: Tensor-network compressed quantum phase estimation algorithm achieves scalable, accurate energy gap estimation for Hubbard models using error mitigation and iterative optimization, demonstrated on up to 52-qubit systems.


<details>
  <summary>Details</summary>
Motivation: To develop a scalable quantum phase estimation algorithm that overcomes limitations of conventional QPE by using tensor-network circuit compression to enable larger system simulations on near-term quantum devices while maintaining accuracy.

Method: Uses tensor networks to construct circuits with only nearest-neighbor gates, extracts time-evolution data via four-type circuit measurements, employs algorithmic error mitigation for time-evolution circuits, and implements iterative circuit optimization combined with matrix product state merging for state preparation.

Result: Achieves 0.4-4.7% error from true energy gap for 8-qubit 1D Hubbard model on noiseless simulator, shows accuracy improvements from error mitigation, confirms enhanced overlap with matrix product states through optimization, and successfully demonstrates on IBM Heron devices for 8-, 36-, and 52-qubit models using over 5,000 2-qubit gates.

Conclusion: The proposed tensor-network compressed QPE algorithm represents significant progress toward practical near-term quantum computing applications and preparation for error-corrected quantum devices, demonstrating scalability to large system sizes while maintaining accuracy through innovative error mitigation and optimization techniques.
...
TL;DR: Quantum computing-based failure detection algorithm using projected quantum feature maps and statistical change-point detection for industrial machine monitoring, validated on benchmark and real-world IoT sensor data using IBM's 133-qubit Heron processor.


<details>
  <summary>Details</summary>
Motivation: Prompt detection of machine failures is crucial for industrial efficiency and minimizing downtime. There's a need for more precise anomaly detection methods in machine monitoring systems, and quantum computing offers potential advantages for enhancing detection accuracy.

Method: Developed a failure detection algorithm combining quantum computing with statistical change-point detection. Uses projected quantum feature maps to enhance anomaly detection precision. Implemented and executed on IBM's 133-qubit Heron quantum processor.

Result: Empirically validated on benchmark multi-dimensional time series datasets and real-world IoT sensor readings from operational machines. Demonstrated effectiveness in accurately identifying anomalies in noisy time series data. Showed feasibility of integrating quantum computing into industrial maintenance procedures.

Conclusion: The quantum-based failure detection system is effective for industrial diagnostics. This work highlights the potential of quantum computing in predictive maintenance and paves the way for more sophisticated quantum algorithms in this domain.
...
TL;DR: Photon-added cat and kitten states show metrological advantage over original forms due to phase-space broadening from increased amplitude, though with higher energy cost. Squeezed states and superposed states constructed from weak squeezing and displacement are compared using quantum Fisher information and fidelity.


<details>
  <summary>Details</summary>
Motivation: To investigate whether photon addition to cat and kitten states can enhance their metrological performance for phase estimation, despite the increased energy cost, and to explore regimes where these enhanced states can be prepared using accessible Gaussian operations.

Method: Construct squeezed states and two superposed states (squeezed cat state and symmetrically squeezed state) using weak squeezing and displacement operations. Create photon-added variants and compare with parity-matched cat and kitten states using quantum Fisher information (QFI) and fidelity analysis. Analyze QFI isocontours to identify regimes of high fidelity and large amplitude.

Result: Photon-added cat and kitten states demonstrate metrological advantage over original forms due to phase-space broadening from increased amplitude. QFI isocontours reveal regimes where kitten states exhibit high fidelity and large amplitude, enabling preparation via Gaussian operations and photon addition. Similar regimes exist for cat states enhanced by squeezing and photon addition, showing improved metrological performance.

Conclusion: Photon addition enhances the metrological performance of cat and kitten states through phase-space broadening and increased amplitude, despite higher energy cost. The identified regimes enable practical preparation using accessible Gaussian operations. Additionally, increased amplitude reduces interferometric fringe size, potentially enhancing quantum error correction effectiveness in cat codes.
...
TL;DR: Hybrid quantum-classical workflow using ONIOM framework on Fugaku supercomputer and Quantinuum quantum computer for accurate biomolecular reaction simulations


<details>
  <summary>Details</summary>
Motivation: To enable scalable and accurate simulation of complex biomolecular reactions by extending the layered approach to treat both the active site (protein) and large molecular environment

Method: Developed a workflow within the ONIOM framework using a hybrid computing system combining the Fugaku supercomputer (classical) and Quantinuum Reimei trapped-ion quantum computer (quantum)

Result: Demonstrated successful implementation on the hybrid platform, extending the layered approach to accurately treat both active site and molecular environment

Conclusion: Represents a significant milestone in enabling scalable and accurate simulation of complex biomolecular reactions through quantum-classical hybrid computing
...
TL;DR: Generalization of squeezing problem to fractional squeezing orders enables precise identification of critical points where qualitative behavior changes occur, including continuous-to-discrete spectrum transitions and infinite-to-finite amplitude oscillation transitions.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of conventional computational methods in identifying critical points where qualitative behavior changes occur in squeezing problems, particularly those involving transitions between continuous/discrete spectra and infinite/finite amplitude oscillations.

Method: Generalize the generalized-squeezing problem to include fractional values of squeezing order n, enabling analytical determination of critical point locations and accurate prediction of behavior at these points, supplemented by numerical calculations.

Result: Successfully identify with high confidence the critical point where spectrum transitions from continuous to discrete, and the point where oscillations transition from asymptotically infinite amplitudes to finite amplitudes. Provide intuitive explanation for large n regime behavior consistent with numerical results.

Conclusion: Fractional generalization of squeezing order provides powerful analytical framework for identifying and characterizing critical points in squeezing problems, overcoming limitations of conventional computational methods and offering insights into qualitative behavior transitions.
...
TL;DR: Unsplit spreading of localized excitations requires singular dispersion relations enabled by long-range interactions, observable in quantum simulation experiments and realistic open quantum systems.


<details>
  <summary>Details</summary>
Motivation: To explain why localized excitations in conventional lattice models split into counter-propagating wave packets, and to identify the conditions under which unsplit spreading can occur as a distinct physical effect.

Method: Theoretical proof that smooth dispersion relations cause splitting, analysis of long-range interactions enabling singular dispersion features, and demonstration using realistic open quantum systems like 1D and 2D subwavelength atomic arrays with long-lived subradiant states.

Result: Unsplit spreading occurs only when dispersion relations develop singularities, which long-range interactions enable. This phenomenon was visible in published quantum simulation experiments since 2014 but unrecognized. Realistic systems like subwavelength atomic arrays exhibit unsplit spreading due to singular effective dispersion in subradiant states.

Conclusion: Unsplit spreading serves as an experimentally accessible signature of singular band structure induced by long-range physics, establishing it as a distinct physical effect with implications for understanding wave propagation in quantum systems.
...
TL;DR: Parameter transfer initialization with targeted single-layer QAOA optimization achieves near-optimal performance (98.88%) with 8.06× speedup for unweighted graphs, but struggles with weighted graphs; ridge regularization reduces optimization inconsistencies from 8.92% to 3.81%.


<details>
  <summary>Details</summary>
Motivation: To improve the efficiency of QAOA for combinatorial optimization problems by developing parameter initialization and optimization strategies that reduce computational cost while maintaining solution quality, particularly for the MaxCut problem across different graph families.

Method: Parameter transfer initialization followed by targeted single-layer optimization for QAOA applied to MaxCut on 3-regular, Erdős–Rényi, and Barabási–Albert graphs; comparison with full optimization; use of ridge (L2) regularization to smooth parameter landscape and reduce optimization inconsistencies.

Result: For unweighted graphs: mean approximation ratio of 0.9443 vs 0.9551 for full optimization (98.88% optimal performance) with 8.06× computational speedup. For weighted graphs: performance <90% for higher nodes, except weighted 3-regular graphs. Targeted optimization outperformed full optimization in 8.92% of cases; ridge regularization reduced this to 3.81%.

Conclusion: Efficient parameter initialization and targeted single-layer optimization can significantly improve QAOA efficiency with minimal performance degradation for unweighted graphs, but weighted graphs require different approaches; ridge regularization helps mitigate optimization inconsistencies in complex parameter landscapes.
...
TL;DR: LPD algorithm efficiently approximates local observables for short-time noiseless quantum dynamics using low-weight Pauli truncation, with error bounds that improve with entanglement.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between tensor-network methods (inefficient at high entanglement) and Pauli-truncation approaches (typically requiring noise/randomness) for classical simulation of noiseless quantum dynamics.

Method: Low-weight Pauli Dynamics (LPD) algorithm that truncates high-weight Pauli operators while maintaining accuracy for local observables in short-time dynamics, with error bounds proven without assuming randomness when states are sufficiently entangled.

Result: Counterintuitive finding that entanglement alleviates classical simulation error; entangled states can be generated by tensor-network simulation or near-term quantum devices; establishes synergy between classical methods and reduces circuit depth requirements for long-time dynamics.

Conclusion: LPD provides a rigorous complementary route to quantum simulation that extends the accessible regime of quantum dynamics by reducing circuit depth requirements and creating synergy between existing classical simulation approaches.
...
TL;DR: The paper proposes a family of quantum walks free from fermion doublers and pseudo-doublers that still simulate the Dirac equation in the continuum limit, achieved by allowing non-zero probability for the walker to stay at the same point.


<details>
  <summary>Details</summary>
Motivation: Fermion doubling in quantum walks used to simulate Dirac particles creates spurious low-energy solutions that persist in second-quantized versions (quantum cellular automata) and cause problems when introducing interactions. Pseudo-doublers also threaten vacuum stability.

Method: Propose a family of quantum walks that eliminate doublers and pseudo-doublers while maintaining Dirac equation simulation in continuum limit. Key modification: allow non-zero probability for walker staying at same point (unlike conventional Dirac walks which always have zero probability for staying).

Result: Successfully constructs quantum walks free from doublers and pseudo-doublers. However, a small number of additional low-energy solutions remain that don't directly correspond to Dirac particles. The walks still simulate Dirac equation in continuum limit despite the modifications.

Conclusion: The proposed family of quantum walks addresses the fermion doubling problem and pseudo-doubler issues while preserving Dirac equation simulation, though some non-Dirac low-energy solutions persist. The key innovation is relaxing the zero-probability constraint for staying at the same point.
...
<details>
  <summary>Details</summary>
Motivation: To improve cryptographic security in quantum teleportation by leveraging the mathematical framework of q-deformed algebras, which introduces additional parameters that must be shared between parties for successful decryption.

Method: Construct q-deformed harmonic oscillator states and Bell-like entangled states that reduce to standard quantum states when q→1. Develop two new teleportation protocols using q-deformed non-maximally entangled states with additional parameters that must be shared for decryption.

Result: Successfully created q-deformed Bell-like states that form an orthonormal basis for bipartite entangled states under certain constraints. Developed two new teleportation protocols with enhanced cryptographic security due to the requirement of sharing additional deformation parameters for decryption.

Conclusion: The q-deformed algebraic framework provides a systematic way to enhance cryptographic security in quantum teleportation by introducing additional parameters that must be securely shared between communicating parties, offering improved protection against eavesdropping.
...
<details>
  <summary>Details</summary>
Motivation: Current quantum circuit optimization focuses on exact unitary transformations, but substantial resource reduction opportunities exist by allowing approximate local transformations and employing mixed quantum channels to approximate pure circuits.

Method: Refined ZX-calculus-based automated optimization with greedy strategy that selectively replaces ZX-diagrams with small phase angles using stochastic mixtures of identity and carefully chosen over-rotations, designed to reduce gate count in expectation while staying within strict error budget.

Result: Achieves modest two-qubit gate reduction in random circuits and substantial reduction in structured circuits like quantum Fourier transform; converts experimental noise into deliberately engineered random noise, outperforming other approximation methods on average.

Conclusion: Mixed-channel approximations show potential to enhance quantum circuit performance, suggesting new directions for resource-aware automated quantum compilation beyond pure unitary channels.
...
<details>
  <summary>Details</summary>
Motivation: To understand how frictional work (work loss due to non-adiabatic driving) relates to quantum coherence and entropy production in quantum systems, and how integrability affects these relationships across different driving regimes.

Method: Analyze frictional work in non-integrable and integrable spin chains by comparing work output between adiabatic and non-adiabatic driving protocols, relating frictional work to diagonal entropy production (for slow/moderate driving) and quantum relative entropy (for fast driving), and examining effective temperature descriptions.

Result: In non-integrable spin chains, frictional work equals diagonal entropy production times effective temperature for slow/moderate driving, and equals quantum relative entropy between final states for fast driving. In integrable chains, frictional work decomposes into sum over independent subspaces at different effective temperatures. Integrability breaking enhances work extraction in adiabatic limit but degrades it in sufficiently non-adiabatic regimes.

Conclusion: Frictional work in quantum systems has distinct relationships with entropy measures depending on driving speed: diagonal entropy production for slow/moderate driving and quantum relative entropy for fast driving. Integrability strongly influences these relationships, with integrability breaking improving adiabatic work extraction but harming non-adiabatic performance, highlighting the interplay between coherence, integrability, and work extraction efficiency.
...
TL;DR: Non-perturbative derivation of renormalization relations for waveguide-QED models that connect bare parameters to physical observables, enabling efficient numerical simulations with minimal bandwidth.


<details>
  <summary>Details</summary>
Motivation: Numerical simulations of waveguide-QED models require introducing IR and UV cutoffs, creating a discrepancy between bare model parameters and physically observable quantities. There's a need for a systematic approach to relate these parameters while maintaining physical accuracy in simulations.

Method: Formulate atomic dynamics in the time domain to derive renormalization relations non-perturbatively from first principles. The approach explicitly accounts for IR/UV cutoffs, verifies consistency with scattering theory, and connects results to standard Feynman diagrams for physical interpretation.

Result: Obtained explicit expressions linking bare model parameters to physically observable atomic frequency and decay rate. Demonstrated consistency with scattering theory and provided transparent physical interpretation through connection to Feynman diagrams. Showed how these relations enable parameterization of simulations with minimal frequency bandwidth while preserving accuracy.

Conclusion: The derived renormalization relations provide a systematic framework for efficient waveguide-QED simulations, reducing computational cost while maintaining physical accuracy. This enables reliable multi-photon light-matter simulations by properly handling cutoff effects in numerical implementations.
...
TL;DR: Universal digitized counterdiabatic driving method that constructs adiabatic gauge potential digitally without introducing many-body/nonlocal interactions, incorporates infinite nested commutators, and provides explicit rotation angles.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of existing universal counterdiabatic driving and digitized counterdiabatic driving methods, particularly the introduction of many-body/nonlocal interactions and inability to fully incorporate infinite nested commutators that constitute the adiabatic gauge potential.

Method: Proposes a universal method of digitized counterdiabatic driving that constructs the adiabatic gauge potential in a digital way using the idea of universal counterdiabatic driving, enabling digital implementation with explicit rotation angles.

Result: The method is shown to be consistent with exact theory analytically and demonstrates effectiveness through numerical simulations, offering advantages over existing approaches.

Conclusion: The proposed universal digitized counterdiabatic driving method provides a practical approach for quantum control without introducing problematic interactions while fully incorporating the mathematical structure of adiabatic gauge potentials.
...
TL;DR: Semiclassical framework for entanglement dynamics in spin-boson systems using complex classical trajectories to accurately capture entanglement entropy beyond Ehrenfest time.


<details>
  <summary>Details</summary>
Motivation: To develop a semiclassical description of entanglement dynamics between spin and bosonic field subsystems, going beyond conventional semiclassical methods that typically fail for entanglement calculations, especially beyond the Ehrenfest time.

Method: Uses a semiclassical framework with initial product state of spin coherent state and canonical coherent state. Derives entanglement entropy expression dependent on classical trajectories. Extends phase space to complex domain to identify additional complex trajectories that improve accuracy. Demonstrates approach with representative example showing role of real and complex trajectories.

Result: Complex trajectories significantly improve accuracy of semiclassical entanglement description. The method captures entanglement dynamics with remarkable precision, even well beyond the Ehrenfest time where conventional semiclassical methods typically fail.

Conclusion: Analytical extension of classical phase space into complex domain provides powerful framework for semiclassical description of entanglement dynamics, enabling accurate calculation of entanglement entropy using both real and complex classical trajectories.
...
TL;DR: The paper proposes using the Liouvillian skin effect to engineer the quantum Mpemba effect in open quantum systems, revealing a new type of QME with two distance reversals.


<details>
  <summary>Details</summary>
Motivation: To develop a new approach for engineering the quantum Mpemba effect (where distant initial states relax faster than closer ones) using the Liouvillian skin effect as an ideal platform that also facilitates experimental preparation.

Method: Focus on quadratic Lindbladians and consider two concrete cases to design initial states that realize the QME. The Liouvillian skin effect's spatial profile provides a straightforward pathway for initial state preparation.

Result: Discovery of a new kind of QME (QME-III) distinct from typical scenarios, characterized by two reversals in the Hilbert-Schmidt distance at two different times. The LSE provides a physically intuitive understanding of the QME.

Conclusion: The Liouvillian skin effect serves as an effective platform for engineering the quantum Mpemba effect, enabling experimental realization and providing deeper physical insight into the phenomenon.
...
<details>
  <summary>Details</summary>
Motivation: To benchmark quantum hardware performance by implementing operational constraints for evaluating detectability of non-ideal channels relative to calibrated device noise, rather than testing quantum interpretations.

Method: Implemented a five-qubit instance of Violaris' protocol as an inter-register message-transfer pattern within a single circuit on IBM Quantum hardware (ibm_fez backend). The circuit encodes branch-conditioned evolution of an observer subsystem with control qubit dependency, followed by controlled transfer operations probing correlations between conditional measurement contexts. Executed with 20000 shots.

Result: Observed population-based visibility of 0.877, coherence witnesses of 0.840 and -0.811 along orthogonal axes, and phase-sensitive magnitude of approximately 1.17. The visibility metric is insensitive to some dephasing classes while coherence witnesses provide complementary sensitivity to off-diagonal noise.

Conclusion: The work establishes a reproducible operational constraint pipeline for evaluating quantum hardware performance relative to calibrated noise, demonstrating practical implementation of inter-branch communication witness estimation without addressing quantum interpretation questions.
...
TL;DR: Echoed random process achieves near-Heisenberg-limit sensitivity without requiring carefully prepared quantum probe states or complex quantum controls, using random driving of Kerr nonlinear modes to create sub-Planck structures.


<details>
  <summary>Details</summary>
Motivation: Quantum metrology traditionally requires exotic quantum states (entangled/squeezed) and precise system calibration/optimization, which limits scalability and robustness. The paper aims to circumvent these limitations.

Method: Introduces an echoed random process that drives a Kerr nonlinear mode with random pulses, creating sub-Planck phase-space structures that enable high sensitivity without requiring knowledge of the random probe state or complex quantum control.

Result: Achieves sensitivity approaching the Heisenberg limit while being statistically robust across broad driving parameter ranges, resilient to control fluctuations and photon loss, and applicable to both bosonic and qubit platforms.

Conclusion: Provides a practical, hardware-efficient, scalable, and optimization-free route to quantum-enhanced metrology in high-dimensional Hilbert spaces, eliminating traditional requirements for exotic quantum states and precise control.
...
TL;DR: GQSPI framework for binary hypothesis testing of displacement parameters in hybrid qubit-bosonic systems using polynomial approximation, achieving O(1/d log d) error probability with robustness to noise.


<details>
  <summary>Details</summary>
Motivation: Need to determine if a displacement parameter β falls within asymmetric thresholds [β_{-th}, β_{+th}] in quantum sensing, which is a binary decision problem for quantum signals.
...
Result: Achieves small decision error probability p_err ~ O(1/d log d) with circuit depth d; works for deterministic β and random β with known prior; robust to dephasing noise; extends to multi-threshold cases.

Conclusion: Framework enables decision-making over arbitrary thresholds for general displacement signals in single or few shots, providing efficient quantum hypothesis testing.
...
TL;DR: Quantum metrology with coarse-grained measurements achieves Heisenberg scaling even with only 2-bin detection, demonstrating practical quantum enhancement under experimental imperfections.


<details>
  <summary>Details</summary>
Motivation: While quantum metrology offers precision beyond classical limits, its performance is typically vulnerable to experimental imperfections. Most prior work focused on imperfections in quantum states and operations, but this study investigates the effect of coarse graining in quantum measurement itself.

Method: Theoretical analysis and experimental demonstration using an interferometer with squeezed vacuum and laser input. Analyzed how coarse graining in homodyne detection affects phase estimation precision. Evaluated Fisher information under various coarse-graining conditions and determined optimal estimation strategies that saturate the Cramér-Rao bound. Employed method of moments with calibration procedures for general experimental settings.
...
Conclusion: Coarse-grained quantum measurements can achieve quantum-enhanced precision despite severe experimental imperfections, providing a practical pathway for implementing quantum metrology in real-world settings with measurement limitations.
...
TL;DR: The paper proposes a measurement-based feedback approach for quantum prime factorization that eliminates classical parameter computation, experimentally demonstrating factorization of 551 with NMR and numerically scaling to larger numbers.


<details>
  <summary>Details</summary>
Motivation: Existing quantum factorization methods either require high-fidelity gates (Shor's algorithm) or substantial classical post-processing (Hamiltonian optimization methods). The authors aim to develop an all-quantum approach that eliminates classical computation of control parameters while maintaining scalability.

Method: An all-quantum, measurement-based feedback approach that iteratively steers a quantum system toward the target ground state where prime factors are encoded as degenerate ground states of a problem Hamiltonian. The method uses measurement feedback to determine control parameters without classical computation.

Result: Experimental factorization of biprime 551 using a three-qubit NMR quantum register, with numerical analysis showing robustness against control field errors. Numerical demonstration of scalability by factoring larger biprimes 9,167 (5 qubits) and 2,106,287 (9 qubits) using the FALQON factorization method.

Conclusion: The proposed measurement-based feedback approach provides a viable alternative to existing quantum factorization methods by eliminating classical parameter computation while maintaining scalability and robustness against control errors.
...
TL;DR: The paper extends the noise-independence principle for Liouvillian spectra to multimode bosonic Gaussian systems in both continuous and discrete time, showing that eigenvalues and non-diagonalizability are controlled by drift while diffusion determines steady states and eigenoperator structure.


<details>
  <summary>Details</summary>
Motivation: To generalize McDonald and Clerk's finding that Liouvillian spectra are independent of noise strength in linear open quantum systems, specifically extending this principle to multimode bosonic Gaussian Markov semigroups and discrete-time Gaussian channels.

Method: For continuous-time systems: construct a time-independent Gaussian similarity transformation fixed by the Lyapunov equation that gauges away diffusion. For discrete-time systems: construct explicit Gaussian similarity transformation that gauges away diffusion at the channel parametrization level. Analytical demonstration with single-mode squeezed-reservoir Lindbladian and non-Markovian Gaussian channels.

Result: Established that eigenvalues and non-diagonalizability of Gaussian systems are entirely controlled by drift, while diffusion determines steady states and eigenoperator structure. Derived explicit gauging transformations for both continuous and discrete time cases. Obtained analytical expressions for exceptional-point manifolds and associated gauging covariances.

Conclusion: The noise-independence principle holds for multimode bosonic Gaussian systems in both continuous and discrete time, with drift controlling spectral properties and diffusion determining steady-state properties, enabling systematic analysis through explicit gauging transformations.
...
TL;DR: A hardware-conditioned neural decoder framework for quantum error correction that separates calibration processing from low-latency decoding, achieving 11.1x reduction in logical error rate on unseen experiments without retraining.


<details>
  <summary>Details</summary>
Motivation: Real-time quantum error correction decoding requires high accuracy, low latency, and robustness to hardware noise variations. Superconducting processors have natural separation of timescales: calibration drifts occur over hours while error correction requires microsecond responses.

Method: Hardware-conditioned neural decoder framework with graph-based encoder for calibration data and lightweight convolutional backbone conditioned via feature-wise linear modulation (FiLM). This decouples heavy processing of device statistics from low-latency syndrome decoding.

Result: Evaluation on IBM Fez, Kingston, and Pittsburgh processors with over 2.7 million experimental shots (distances up to d=11). Single trained model generalizes to unseen qubit chains and new calibration data days later without retraining. Achieves up to 11.1x reduction in logical error rate relative to modified minimum-weight perfect matching.

Conclusion: Hardware-conditioned neural decoding demonstrates promising adaptive performance with negligible latency overhead by exploiting the asynchronous nature of system calibration and decoding, enabling practical real-time quantum error correction.
...
TL;DR: Quantum HMM compression pipeline enables dimension reduction for any finite ergodic HMM, outperforming classical approaches in memory-accuracy trade-offs.


<details>
  <summary>Details</summary>
Motivation: Classical HMMs are large and high-dimensional, while existing quantum HMM compression techniques are limited to deterministic transition HMMs, creating a need for general compression methods.

Method: Developed a pipeline that compresses any finite, ergodic HMM using quantum dimension reduction techniques, demonstrated on toy models and speech-derived HMMs.

Result: The method achieves favorable memory-accuracy trade-offs compared to classical compression approaches, enabling effective quantum dimension reduction for general HMMs.

Conclusion: The proposed pipeline successfully extends quantum HMM compression to arbitrary finite ergodic HMMs, providing a practical route for quantum dimension reduction with superior performance over classical methods.
...
TL;DR: SBO-QAOA addresses bias in degenerate ground state sampling in QAOA by using a temperature-dependent Hamiltonian to achieve uniform distribution among degenerate solutions.


<details>
  <summary>Details</summary>
Motivation: Standard QAOA with transverse-field mixer introduces biases among degenerate ground states as circuit depth increases, preventing fair sampling of all optimal solutions in combinatorial optimization problems.

Method: SBO-QAOA employs a temperature-dependent Hamiltonian encoding a Gibbs distribution as its ground state, based on quantum-classical correspondence theory. It uses only four variational parameters under a linear schedule.

Result: Numerical simulations show SBO-QAOA yields ground-state probabilities converging to finite-temperature values with uniform distribution among degenerate states, unlike standard QAOA which exhibits biases.

Conclusion: SBO-QAOA achieves both fairness in degenerate state sampling and temperature-targeting properties, maintaining these advantages even with minimal variational parameters and simple scheduling.
...
TL;DR: The paper proves polynomial-time convergence for two families of Lindbladian processes modeling quantum Gibbs sampling and open many-body thermalization, demonstrating efficient preparation of complex Gibbs states and accurate modeling of thermal relaxation.


<details>
  <summary>Details</summary>
Motivation: Understanding convergence speed of Lindbladians modeling weakly-coupled quantum systems is crucial for bounding algorithmic runtimes and thermalization timescales in quantum Gibbs sampling and open many-body quantum thermalization.

Method: Study two families of Lindbladian processes: repeated-interaction Gibbs sampling algorithm and open many-body quantum thermalization models. Use novel technical result extrapolating spectral gap lower bounds from quasi-local Lindbladians to non-local generators.

Result: Prove polynomial-time convergence for both processes in several non-commuting systems including high-temperature local lattices, weakly interacting fermions, and 1D spin chains.

Conclusion: Simple dissipative quantum algorithms can prepare complex Gibbs states efficiently, and Lindblad dynamics accurately capture thermal relaxation in quantum systems.
...
TL;DR: Stabilizer-based approach for constructing analytic eigenstates of nonintegrable Hamiltonians reveals fundamental limitation: stabilizer eigenstates of two-body Hamiltonians cannot achieve k-body microscopic thermal equilibrium for k≥4, with tight bound demonstrated through explicit constructions.


<details>
  <summary>Details</summary>
Motivation: To develop analytical tools for understanding highly entangled thermal eigenstates in quantum many-body systems, particularly addressing the challenge of analyzing such states in nonintegrable Hamiltonians.

Method: Introduces a stabilizer-based approach to construct analytically tractable energy eigenstates, focusing on zero-energy eigenstates at infinite temperature. Proves a no-go theorem about limitations of stabilizer eigenstates and constructs explicit examples to demonstrate tightness of bounds.

Result: Proves sharp no-go theorem: stabilizer eigenstates of two-body Hamiltonians cannot satisfy k-body microscopic thermal equilibrium for any k≥4. Shows bound is tight by constructing two-body nonintegrable Hamiltonians whose stabilizer eigenstates reproduce thermal expectation values for all two-body and three-body observables.

Conclusion: Reveals fundamental constraint imposed by few-body nature of interactions on stabilizer states appearing as zero-energy eigenstates, providing structural understanding of limitations in achieving microscopic thermal equilibrium with stabilizer eigenstates in few-body Hamiltonians.
...
TL;DR: GKP encoding enables homodyne-based Bell tests in continuous-variable systems, showing bipartite no-go for CHSH violation but demonstrating multipartite nonlocality with finitely squeezed GKP-encoded GHZ and W states.


<details>
  <summary>Details</summary>
Motivation: Bell tests using homodyne detection face constraints in continuous-variable systems. The paper investigates whether Gottesman-Kitaev-Preskill (GKP) encoding can transform homodyne detection into a practical tool for revealing Bell nonlocality, addressing the limitations of continuous-variable Bell tests.

Method: The authors consider a physically motivated model where each party performs homodyne detection and digitizes continuous outcomes via fixed periodic binning (corresponding to logical Pauli measurements). They derive bipartite no-go theorems for CHSH violation and analyze multipartite scenarios using GKP-encoded GHZ and W states with finite squeezing.

Result: 1. Bipartite no-go: CHSH cannot be violated for Bell-pair states within the homodyne-only framework. 2. Multipartite success: Finitely squeezed GKP-encoded GHZ and W states exhibit strong multipartite nonlocality, violating multipartite Bell inequalities with homodyne-only readout. 3. Quantification: The authors determine required squeezing thresholds and robustness to loss.

Conclusion: GKP encoding enables homodyne-based Bell tests in continuous-variable systems, overcoming bipartite limitations through multipartite approaches. The work provides a practical route for Bell tests using only homodyne detection by quantifying squeezing requirements and loss tolerance for GKP-encoded states.
...
TL;DR: Physical KANs using trained synaptic nonlinearities outperform linear weight-based networks with far fewer parameters and devices, demonstrated experimentally with silicon SYNE devices.


<details>
  <summary>Details</summary>
Motivation: Traditional physical neural networks train only linear synaptic weights while treating device nonlinearities as fixed, missing opportunities to exploit physical nonlinear dynamics for more efficient computation.

Method: Implemented Kolmogorov-Arnold Network (KAN) architectures that train synaptic nonlinearities themselves, realized experimentally in silicon-on-insulator devices called Synaptic Nonlinear Elements (SYNEs) operating at room temperature with 0.1-1 μA currents and 2 MHz speeds.

Result: Physical KANs outperformed equivalently-parameterized software multilayer perceptron networks across nonlinear function regression, classification, and Li-Ion battery dynamics prediction tasks, with up to 100× fewer parameters and 100× fewer devices than linear weight-based physical networks.
...
TL;DR: The paper extends Gioia and Wang's momentum-LRE connection to non-translation-symmetric 1D systems using the translation operator expectation value as a proxy for long-range entanglement, analogous to Resta's formula for localization length.


<details>
  <summary>Details</summary>
Motivation: Gioia and Wang showed that translationally symmetric states with nonzero momentum are necessarily long-range entangled (LRE), but this result requires finding finite-depth quantum circuits connecting states to translation-symmetric ones, which is impractical for translation-symmetry-breaking states. The authors aim to develop a momentum-based entanglement probe that works directly for non-translation-symmetric states.

Method: Instead of translation eigenstates, the authors analyze the many-body momentum distribution and expectation value of the translation operator ⟨T⟩ in broken-translation-symmetry states. They show that in the continuum limit, |⟨T⟩| → 1 for delocalized states (proxy for LRE in 1D). They test this approach on lattice models: a deterministic random dimer model to study thermodynamic/continuum limits, a simplified Aubry-André model with commensurate hopping, and the random dimer model as a test case for periodic lattice models without well-defined continuum limits.

Result: The answer is affirmative for 1D systems: momentum can encode entanglement nature for non-translation-symmetric states. In continuum limit, |⟨T⟩| → 1 for delocalized states, serving as a momentum-space analog of Resta's localization length formula. Higher-dimensional extensions and topologically ordered systems require further investigation.

Conclusion: The expectation value of the translation operator |⟨T⟩| serves as a practical momentum-space probe for long-range entanglement in 1D systems without translation symmetry, extending Gioia and Wang's result to more realistic scenarios where finding connecting quantum circuits is impractical.
...
TL;DR: Mobility edge positions in quasiperiodic systems are structurally constrained by isospectral duality, forcing them to specific energies and leading to linear critical scaling near self-dual points.


<details>
  <summary>Details</summary>
Motivation: To understand that mobility edge positions are not independent spectral features but are structurally constrained across quasiperiodic Hamiltonians related by isospectral duality, moving beyond individual Hamiltonian analysis.

Method: Using a bichromatic Aubry-André model as minimal setting, demonstrating constraint encoded in exact identity for Lyapunov exponents derived from Thouless formula, analyzing mobility edge positions and critical scaling.

Result: Mobility edge positions are restricted to reduced set of energies; in self-dual limit they coincide at single localization-delocalization transition; structural constraint enforces linear critical scaling of physical Lyapunov spectrum near self-dual point with critical exponent ν=1 and novel non-universal energy-dependent prefactor.

Conclusion: Mobility edges in quasiperiodic systems are fundamentally constrained by isospectral duality relations, revealing structural connections between different Hamiltonians and providing new insights into localization transitions and critical scaling behavior.
